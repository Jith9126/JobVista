...
public class OpeningDetailsDAO {
	...
	public List<JSONObject> getOpeningsForPanelist(int PanelistID) {
		List<JSONObject> openingDetailsList = new ArrayList<>();
		try (Connection connection = ConnectionClass.CreateCon().getConnection()) {
			String query = "SELECT o.Opening_Id AS Opening_Id, o.Department_Id AS Opening_Department_Id, "
					+ "o.Description AS Opening_Description, o.Experience AS Opening_Experience, "
			...
			try (ResultSet resultSet = preparedStatement.executeQuery()) {
				while (resultSet.next()) {
					JSONObject openingDetailsJSON;
					try {
						openingDetailsJSON = mapResultSetToJSONForPanelist(resultSet);
						openingDetailsList.add(openingDetailsJSON);
					} catch (JSONException e) {
						e.printStackTrace();
					}
	...
	private JSONObject mapResultSetToJSONForPanelist(ResultSet resultSet) throws SQLException, JSONException {
		JSONObject openingDetailsJSON = new JSONObject();
		openingDetailsJSON.put("Opening", getOpeningDetails(resultSet));
		openingDetailsJSON.put("Panelist", getPanelistDetails(resultSet));
		openingDetailsJSON.put("Organization", getOrganizationDetails(resultSet));
...
//	private JSONObject getallPanelistDetailsFromOpening(ResultSet resultSet) throws SQLException, JSONException {
//		JSONObject getallPanelistDetailsFromOpeningJSON = new JSONObject();
//
//		getallPanelistDetailsFromOpeningJSON.put("departmentId", resultSet.getLong("Department_Department_Id"));
//		getallPanelistDetailsFromOpeningJSON.put("departmentTitle", resultSet.getString("Department_Title"));
//		getallPanelistDetailsFromOpeningJSON.put("departmentOrgId", resultSet.getLong("Department_Org_Id"));
//
//		return getallPanelistDetailsFromOpeningJSON;
//	}
	private JSONArray getPanelistDetailsByOpeningId(long openingId) throws SQLException, JSONException {
	    ...
	    String panelistQuery = "SELECT p.* FROM Panelist p "
	            + "JOIN OpeningAndPanelist op ON p.Panelist_Id = op.Panelist_Id "
	            + "WHERE op.Opening_Id = ?;";
	    try (Connection connection = ConnectionClass.CreateCon().getConnection();
